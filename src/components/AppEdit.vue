<template>
          ...
          <v-container>
            ...
            <v-row>
                ...
                >
                    ...
                    full-width
                    format="24hr"
                    @click:minute="$refs.timeMenu.save(conferenceData.conference.time)"
                    ></v-time-picker>
                </v-menu>
            </v-row>
            <v-row>
            ...
            <v-row>
              ...
              >
                <GmapMarker
                  v-if="parseFloat(conferenceData.conference.latitude) != 0 
  ...
  export default {
    ...
    methods: {
        ...
        countryChange(event){
            this.conferenceData.conference.latitude = this.countriesLocations[event].lat;
            this.conferenceData.conference.longitude = this.countriesLocations[event].lng;
        },
      saveConf(){
        this.btnsLoading = true;

        let id = this.conferenceData.conference.id;
        this.axios.post("/V1/conference/"+id+'/save', this.conferenceData.conference).then((response) => {
